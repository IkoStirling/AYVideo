#pragma once
// AYVideoTypes.h โ€?core enums + result codes for the AYVideo module.
//
// design.md ยง5 (V0.5): result-code discipline mirrors AYVoxel's VoxelResult
// (Ok = 0, canonical codes, `Count` sentinel appended โ€?never inserted
// mid-list). All public mutators return VideoResult; void failure paths
// are forbidden (design.md ยง18 anti-pattern table).
...
// ---------------------------------------------------------------------------
// Result codes. Canonical list โ€?append-only; never renumber, never insert
...
enum class VideoResult : uint8_t
{
    ...
    Count                // sentinel โ€?number of canonical codes
};

// Human-readable name for diagnostics. Every code must be covered by
// Test_VideoTypes (design.md ยง19).
